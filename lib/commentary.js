...
const pick = (arr) => {
  ...
  do {
    ...
    attempts++;
  } while (choice === _lastPicked && attempts < 5);
  _lastPicked = choice;
  return choice;
};
...
function napoleonMoveMessage(san) {
  ...
  if (piece === 'n') {
    if (Math.random() > 0.6) return null;
    return { text: pick(NAPOLEON_ATTACK_KNIGHT), mood: 'angry' };
  }

  // Bishop moves — ~60% chance, use bishop-specific pool
  if (piece === 'b') {
    if (Math.random() > 0.6) return null;
    return { text: pick(NAPOLEON_ATTACK_BISHOP), mood: 'angry' };
  }
  ...
  if (piece === 'r') {
    ...
    return { text: pick(NAPOLEON_ATTACK_GENERAL), mood: 'angry' };
  }
...
export function generateFirstMoveMessage(move, ghostBook, napoleonMode = false, source = null) {
  ...
  if (pct >= 60) {
    return pick([
      ...
      `${pct}% of my games start this way.${opening ? ` The ${opening}.` : ''} You should be used to this by now.`,
    ]);
  }
  if (pct > 0) {
    return pick([
