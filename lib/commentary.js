...
function napoleonMoveMessage(san) {
  ...
  if (isCheckmate) return pick(NAPOLEON_VICTORY);

  // Always speak on check
  if (isCheck) return pick(NAPOLEON_CHECK);
  ...
  if (piece === 'n') {
    if (Math.random() > 0.6) return null;
    return pick(NAPOLEON_ATTACK_KNIGHT);
  }

  // Bishop moves — ~60% chance, use bishop-specific pool
  if (piece === 'b') {
    if (Math.random() > 0.6) return null;
    return pick(NAPOLEON_ATTACK_BISHOP);
  }
  ...
  if (piece === 'r') {
    ...
    return pick(NAPOLEON_ATTACK_GENERAL);
  }
...
export function generatePlayerMoveReaction(moveData, napoleonMode = false) {
  ...
  if (napoleonMode) {
    ...
    if (isCheck) {
      ...
      return pick(NAPOLEON_BLUNDER);
    }

    // Player captures — sacrifice quotes, ~60% chance
    if (isCapture) {
      if (Math.random() > 0.6) return null;
      return pick(NAPOLEON_SACRIFICE);
    }
...
export function generateGameOverMessage(result, turn, napoleonMode = false) {
  if (napoleonMode) {
    if (result === 'checkmate') {
      if (turn === 'w') return pick(NAPOLEON_DEFEAT);
      return pick(NAPOLEON_VICTORY);
    }
    return pick(NAPOLEON_DRAW);
  }
