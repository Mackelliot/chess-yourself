...
export function generateIntroMessage(napoleonMode = false) {
  if (napoleonMode) {
    return pick([
      'Emperor\'s gambit loaded. Vive la France.',
      'You face Napoleon. Strategy is my birthright.',
      'I conquered Europe. A chessboard is nothing.',
      'The Emperor takes his seat. Prepare yourself.',
      'From Corsica to the crown. Now, to the board.',
      'I have fought a hundred battles. This one will be swift.',
    ]);
  }
  return pick([
  ...
  if (napoleonMode) {
    if (source === 'remusat') {
      return pick([
        ...
        'Nc3. A move I played against Madame de Remusat. It ended... poorly for her.',
        'This opening won me a famous game once. History shall repeat itself.',
      ]);
    }
    return pick([
      opening ? `The ${opening}. A commander always has a plan.` : 'A carefully chosen advance. Every campaign begins with a single move.',
      'The first move is like the first cannon shot. It sets the tone.',
      opening ? `The ${opening}. I\'ve studied every variation.` : 'Let the battle commence.',
    ]);
  }
...
export function generateStockfishMessage(move, napoleonMode = false) {
  if (napoleonMode) {
    ...
    return pick([
      'Uncharted territory. Perfect \u2014 I thrive in chaos.',
      'No map for this battlefield. I\'ll draw my own.',
      'Off the playbook. Time for tactical brilliance.',
      'The rules of war: adapt, attack, never retreat.',
      'A general must improvise. Watch and learn.',
      'No precedent? Good. I make my own.',
      'Unknown position. The fog of war is my ally.',
    ]);
  }
  // ~30% chance of a context message
  if (Math.random() < 0.3) {
...
export function generateGameOverMessage(result, turn, napoleonMode = false) {
  if (napoleonMode) {
    if (result === 'checkmate') {
      if (turn === 'w') {
        return pick([
          'Checkmate? Against Napoleon? Impossible. And yet... well played.',
          'Defeated. Even emperors fall. But I will return from this Elba.',
          'You have won the battle, but never the war.',
        ]);
      }
      return pick([
        'Checkmate. Waterloo this is not. I am victorious.',
        'Checkmate. The Emperor prevails, as always.',
        'Checkmate. You fought bravely. But bravery alone cannot defeat genius.',
        'Checkmate. Another conquest for the Grande Arm\u00e9e.',
      ]);
    }
    return pick([
      'A draw? Napoleon does not draw. This is merely... a ceasefire.',
      'A stalemate. The Treaty of Tilsit all over again.',
      'Neither victory nor defeat. Unacceptable, but temporary.',
    ]);
  }
